...
class EC2SecurityGroupState(nixops.resources.ResourceState):
    ...
    def create_after(self, resources, defn):
        ...
        return {r for r in resources if
                isinstance(r, nixops_aws.resources.vpc.VPCState) or
                isinstance(r, nixops_aws.resources.elastic_ip.ElasticIPState)
               }
                    ...
                    for rule in grp.rules:
                        for grant in rule.grants:
                            ...
                            if grant.cidr_ip:
                                ...
                                group  = nixops_aws.ec2_utils.id_to_security_group_name(self._conn, grant.groupId, self.vpc_id) if self.vpc_id else grant.groupName
            ...
            for rule in new_rules:
                try:
                    if len(rule) == 4:
                        ...
                        if self.vpc_id:
                            args['id']=nixops_aws.ec2_utils.name_to_security_group(self._conn, rule[3], self.vpc_id)
                        else:
                            args['name']=rule[3]
