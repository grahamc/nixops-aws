...
class Route53RecordSetState(nixops.resources.ResourceState):
    ...
    def boto_session(self):
        if self._boto_session is None:
            (access_key_id, secret_access_key) = nixops_aws.ec2_utils.fetch_aws_secret_key(self.access_key_id)
    ...
    def route53_retry(self, f):
        return nixops_aws.ec2_utils.retry(f, error_codes=[ 'Throttling', 'PriorRequestNotComplete' ], logger=self)

    def create_after(self, resources, defn):
        return {r for r in resources if
                isinstance(r, nixops_aws.resources.route53_hosted_zone.Route53HostedZoneState) or
                isinstance(r, nixops_aws.resources.route53_health_check.Route53HealthCheckState) or
