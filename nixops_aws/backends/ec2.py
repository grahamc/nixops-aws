...
from nixops_aws.resources.ebs_volume import EBSVolumeState
...
class EC2State(MachineState, nixops_aws.resources.ec2_common.EC2CommonState):
    ...
    def get_ssh_private_key_file(self):
        ...
        for r in self.depl.active_resources.itervalues():
            if isinstance(r, nixops_aws.resources.ec2_keypair.EC2KeyPairState) and \
                    r.state == nixops_aws.resources.ec2_keypair.EC2KeyPairState.UP and \
    ...
    def connect_boto3(self):
        ...
        self._conn_boto3 = nixops_aws.ec2_utils.connect_ec2_boto3(self.region, self.access_key_id)
    ...
    def connect_route53(self):
        ...
        (access_key_id, secret_access_key) = nixops_aws.ec2_utils.fetch_aws_secret_key(self.route53_access_key_id)
    ...
    def restore(self, defn, backup_id, devices=[]):
        ...
        for device_stored, v in self.sorted_block_device_mapping():
            ...
            if devices == [] or device_real in devices:
                # detach disks
                volume = nixops_aws.ec2_utils.get_volume_by_id(self.connect(), v['volumeId'])
                ...
                if volume:
                    nixops_aws.ec2_utils.wait_for_volume_available(self._conn, volume.id, self.logger)

                # Check if new volume is available.
                nixops_aws.ec2_utils.wait_for_volume_available(self._conn, new_volume.id, self.logger)
    ...
    def create_after(self, resources, defn):
        ...
        return {r for r in resources if
                ...
                isinstance(r, nixops_aws.resources.ebs_volume.EBSVolumeState) or
                isinstance(r, nixops_aws.resources.elastic_ip.ElasticIPState) or
                isinstance(r, nixops_aws.resources.vpc_subnet.VPCSubnetState) or
                isinstance(r, nixops_aws.resources.vpc_route.VPCRouteState) or
                isinstance(r, nixops_aws.resources.elastic_file_system.ElasticFileSystemState) or
                isinstance(r, nixops_aws.resources.elastic_file_system_mount_target.ElasticFileSystemMountTargetState)}
    ...
    def security_groups_to_ids(self, subnetId, groups):
        ...
        if sg_names != [ ] and subnetId != "":
            ...
            groups = map(lambda g: nixops_aws.ec2_utils.name_to_security_group(self._conn, g, vpc_id), groups)
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
        ...
        for device_stored, v in self.block_device_mapping.items():
            if v.get('needsAttach', False):
                volume = nixops_aws.ec2_utils.get_volume_by_id(self._conn, v['volumeId'], allow_missing=True)
                if volume: continue
        ...
        for device_stored, v in defn.block_device_mapping.iteritems():
            ...
            if volume: nixops_aws.ec2_utils.wait_for_volume_available(self._conn, volume.id, self.logger)
    ...
    def _retry_route53(self, f, error_codes=[]):
        return nixops_aws.ec2_utils.retry(f, error_codes = ['Throttling', 'PriorRequestNotComplete']+error_codes, logger=self)
    ...
    def _update_route53(self, defn):
        ...
        self.route53_access_key_id = defn.route53_access_key_id or nixops_aws.ec2_utils.get_access_key_id()
    ...
    def _delete_volume(self, volume_id, allow_keep=False):
        ...
        volume = nixops_aws.ec2_utils.get_volume_by_id(self.connect(), volume_id, allow_missing=True)
        ...
        if instance.state == "pending":
            ...
            for device_stored, v in self.block_device_mapping.items():
                ...
                if device_that_boto_expects not in instance.block_device_mapping.keys() and v.get('volumeId', None):
                    ...
                    volume = nixops_aws.ec2_utils.get_volume_by_id(self.connect(), v['volumeId'], allow_missing=True)
                    if not volume:
